...
export class AdminController {
  ...
  @ApiQuery({ name: 'q', required: false, description: 'Search query', example: 'alice' })
  @ApiQuery({ name: 'sortBy', required: false, enum: ['createdAt', 'username', 'role', 'status'] })
  ...
  async searchUsers(
    ...
    @Query('limit') limit?: string,
    @Query('offset') offset?: string,
    @Query('sortBy') sortBy?: 'createdAt' | 'username' | 'role' | 'status',
